<h1 align="center">Hi 👋, I'm Zer0iq</h1>
<h3 align="center">A beginner dev, want to be a game dev</h3>

<div style="display: flex; justify-content: space-between; align-items: flex-start;">
    <div style="width: 60%;">
        <h3 align="left">What I'm using:</h3>
        <p align="left">
            <b>OS :</b> Arch Linux.<br>
            <b>Editor : </b> Neovim<br>
        </p>
        <h3 align="left">Learning:</h3>
        <p>
            <ul>
                <li>C++
                    <ul>
                        <li>SFML</li>
                    </ul>
                </li>
                <li>CMake</li>
                <li>Math</li>
            </ul>
        </p>
        <h3 align="left">Connect with me:</h3>
        <p align="left">
            𝕏 : <a href="https://x.com/Zen_Zero0?t=athCviqfyiKtzqw6mab4Mw&s=09">zen.zer0</a>
        </p>
    </div>
    <div style="width: 30%; text-align: right;">
        <img src="artpictures.club/view.php?img=https://media.tenor.com/aNHKkEhomm4AAAAC/anime-keyboard.gif" alt="Your Image" style="max-width: 100%; height: auto; border-radius: 10px;">
    </div>
</div>
